# Digital Humanities Resources

## 1. Tools and methodologies for comparative analysis of documents 

### Semantic annotation of documents according to XML TEI standard
- TEI Publisher: https://teipublisher.com/
	- Es. https://teipublisher.com/index.html#tp-featured-project 

- Kiln: https://kiln.readthedocs.io/en/latest/
	- Es. https://kiln.readthedocs.io/en/latest/projects.html
	- Es. http://archiviomonaci.uniroma1.it/

- EFES: https://github.com/EpiDoc/EFES/wiki 
	- Es. https://github.com/EpiDoc/EFES/wiki/Projects-using-EFES

### Semantic annotation of images
- Pelagios - Recogito: https://recogito.pelagios.org/
	- Es. https://pro.europeana.eu/page/issue-12-pelagios 

### Automated reading of documents from photographic records
Via OCR (Optical Character Recognition) or HTR (Handwritten Text Recognition) technologies.
- Transkribus: https://transkribus.eu/

### Text analysis
- Voyant Tools: https://voyant-tools.org/
- Iramuteq: http://www.iramuteq.org/

### Data visualization via maps, timelines, graphs, slideshows
- Palladio: https://hdlab.stanford.edu/palladio/ 
- OpenAtlas: https://openatlas.eu/
- Neatline: https://neatline.org/ 
- RAWGraphs: https://rawgraphs.io/ 
- Nodegoat: https://nodegoat.net/
- Mirador: https://projectmirador.org/

## 2. Tools for online publishing of results

### Digital publishing tools for archives and text collections or multimedia
Possibly organized in digital exhibitions, galleries, showcases.
- Omeka: https://omeka.org/, https://info.omeka.net/showcase/
- xDams: https://www.xdams.org/

### Digital storytelling tools based on maps, timelines, graphs
- StoryMapJS: https://storymap.knightlab.com/
- TimelineJS: https://timeline.knightlab.com/ 
- StorylineJS: http://storyline.knightlab.com/ 
- HyperCities. Thick Mapping in the Digital Humanities: https://www.hypercities.com/ 

### Multimedia digital publishing tools (for individual texts)
Scientific articles, monographs etc., enriched with images, audio, video, navigable contents.
- Scalar: https://scalar.me/anvc/scalar/showcase/

### XML digital publishing tools (for documents collections)
- TEI Publisher: https://teipublisher.com/
- Kiln: https://kiln.readthedocs.io/en/latest/
- EFES: https://github.com/EpiDoc/EFES/wiki 

### Tools for open access online publishing of datasets
- Zenodo: https://zenodo.org/
- AMS Acta (UniBo): https://amsacta.unibo.it/

### Tools for online publishing of data organized in relational databases
- Heurist: http://heuristnetwork.org/ 

### Collaborative transcription / crowdsourcing tools
- FromThePage: https://fromthepage.com/ (crowdsourcing manuscript transcription)

### Other tools for collaborative work
- GitHub: https://github.com/ (GitHub Desktop: https://desktop.github.com/; GitHub Pages: https://pages.github.com/)
- Google Drive: https://www.google.com/drive/

## 3. Principles, standards, shared vocabularies, data models *et sim.*
- Dublin Core Metadata Initiative: https://dublincore.org/
- CIDOC Conceptual Reference Model (CRM): http://www.cidoc-crm.org/
- Europeana Data Model (EDM): https://pro.europeana.eu/page/metadata
- Text Encoding Initiative (TEI): https://tei-c.org/
- International Image Interoperability Framework (IIIF): https://iiif.io/
- Linked Open Data (LOD): https://www.w3.org/DesignIssues/LinkedData.html,  https://5stardata.info/en/
- Lined Open Data Cloud: https://lod-cloud.net/, https://lod-cloud.net/datasets
- Linked Open Vocabularies (LOV): https://lov.linkeddata.es/dataset/lov/
- Simple Knowledge Organization System (SKOS): https://www.w3.org/TR/skos-reference/
- Resource Description Framework (RDF): https://www.w3.org/RDF/
- eXtensible Markup Language (XML): https://it.wikipedia.org/wiki/XML, https://www.w3.org/XML/
- FAIR principles: https://www.go-fair.org/fair-principles/

## 4. Open data sources
- Wikidata: https://www.wikidata.org, SPARQL endpoint https://query.wikidata.org
- DBpedia: https://www.dbpedia.org/resources/sparql/, SPARQL https://dbpedia.org/sparql
- Common Crawl: http://commoncrawl.org/ + Web Data Commons: http://webdatacommons.org/
- Europeana: https://www.europeana.eu, SPARQL: https://sparql.europeana.eu

## 5. Tools and further resources
- The Programming Historian: https://programminghistorian.org/
- TEI by Example: https://teibyexample.org/
- Peripleo: https://peripleo.pelagios.org/
- Pythia: https://colab.research.google.com/drive/16RfCpZLm0M6bf3eGIA7VUPclFdW8P8pZ
- BaseX: https://basex.org/
- TEIGarage (former OxGarage): https://teigarage.tei-c.org/
- Roma: https://roma.tei-c.org/
- CHET-C-JS: https://cds.library.brown.edu/projects/chet-c/chetc.html
- correspSearch: https://correspsearch.net/en/api.html
- GAMS: https://gams.uni-graz.at/
- Pundit: https://thepund.it/
- TEI-Facsimile oXygen plugin: https://github.com/oxygenxml/TEI-Facsimile-Plugin

## 6. DH projects examples
- Fondazione Bruno Kessler DH projects: https://dh.fbk.eu/category/projects/
- Stanford Literary Lab projects (cf. Distant Reading): https://litlab.stanford.edu/
- Catalog of Digital Scholarly Editions: http://www.digitale-edition.de/
- CGRN: http://cgrn.ulg.ac.be/
- Erasmiana: https://www.erasmiana.org/
- Jheronimus Bosch: https://archief.ntr.nl/tuinderlusten/en.html
- Van Gogh Letters: [https://vangoghletters.org](https://vangoghletters.org/)
- Burckhardt Source: [https://burckhardtsource.org](https://burckhardtsource.org/)
- Die Fackel: https://fackel.oeaw.ac.at/
- Welscher Gast: http://digi.ub.uni-heidelberg.de/wgd/index/welscher_gast.html
- Korrespondenz August Wilhelm Schlegels: [https://august-wilhelm-schlegel.de](https://august-wilhelm-schlegel.de/)
- Carl Maria von Weber Gesamtausgabe: [https://www.weber-gesamtausgabe.de](https://www.weber-gesamtausgabe.de/)
- Digital Vatican Library: https://digi.vatlib.it/
- Stasi Mediathek: https://www.stasi-mediathek.de/
- Romantic Circles: https://romantic-circles.org/
- Manuscriptorium: [https://www.manuscriptorium.com](https://www.manuscriptorium.com/)
- Arthur Schnitzler digital: https://www.arthur-schnitzler.de/
- Digital Thoreau: https://digitalthoreau.org/
- The Readers’ Thoreau: https://commons.digitalthoreau.org/
- Faust Edition: [https://faustedition.net/](https://faustedition.net/document?sigil=2_I_H.17&page=2&view=facsimile_text#l5541)
- Alfred Escher: [https://www.briefedition.alfred-escher.ch](https://www.briefedition.alfred-escher.ch/)

## 7. DH research infrastructures
- DARIAH: [https://www.dariah.eu](https://www.dariah.eu/)
- CLARIN (linguistics): [https://www.clarin.eu](https://www.clarin.eu/)
- ARIADNE (archaeology): [https://www.ariadne-research-infrastructure.eu](https://www.ariadne-research-infrastructure.eu/)
- TextGrid (philology): [https://textgrid.de](https://textgrid.de/)
- Huma-Num: [https://www.huma-num.fr](https://www.huma-num.fr/)
- OPERAS: [https://operas-eu.org](https://operas-eu.org/)
- OpenAire: [https://www.openaire.eu](https://www.openaire.eu/)

## 8. DH readings
- Burdick, Anne; Drucker, Johanna; Lunenfeld, Peter; Presner, Todd; Schnapp, Jeffrey, Digital_Humanities, 2012
- Pierazzo, Elena, Digital Scholarly Editing: Theories, Models and Methods, 2014
- Flanders, Julia; Jannidis, Fotis, The shape of data in the digital humanities. Modeling texts and text-based resources, 2019
- Russell, Stuart; Norvig, Peter, Artificial Intelligence, A Modern Approach, 2021
- AI documentation: http://platform.openai.com/docs, https://platform.claude.com/docs, https://huggingface.co/docs
- Digital Humanities Quarterly (dhq): http://www.digitalhumanities.org/dhq/
- RIDE. A Review Journal for Scholarly Digital Editions and Resources: https://ride.i-d-e.de/
- Journal of the TEI (jTEI): https://journals.openedition.org/jtei/
- Zeitschrift für digitale Geisteswissenschaften (ZfdG): http://www.zfdg.de/
- Current Epigraphy (CurE - not only digital): https://currentepigraphy.org/
- digihum.de: http://digihum.de/
- DHd blog: https://dhd-blog.org/

## 9. Mailing lists
- DHd: https://dig-hum.de/dhd-mailingliste
- TEI: https://listserv.brown.edu/archives/cgi-bin/wa?SUBED1=tei-l&A=1
- EpiDoc: https://www.jiscmail.ac.uk/cgi-bin/webadmin?A0=EPIDOC-MARKUP
- eXist-db: https://sourceforge.net/p/exist/mailman/
